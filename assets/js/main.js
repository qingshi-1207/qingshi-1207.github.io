...
(function() {
   ...
   * No custom JavaScript needed for basic carousel functionality
   */

  /**
   * Hero Section Shrink Effect on Scroll
   */
  function initHeroCarouselShrink() {
    const heroSection = document.querySelector('#hero');
    const heroCarousel = document.querySelector('#heroCarousel');
    const heroOverlay = document.querySelector('.hero-overlay');
    
    if (!heroSection || !heroCarousel) return;
    ...
    function updateHeroScale() {
      ...
      // Scale from 1.0 to 0.7 as user scrolls from 0 to windowHeight
      const maxScroll = windowHeight * 1.5; // Extend the scroll range
      const scrollProgress = Math.min(scrollY / maxScroll, 1);
      
      // Use easing function for smoother animation
      const easeOutCubic = 1 - Math.pow(1 - scrollProgress, 3);
      const scale = 1 - (easeOutCubic * 0.3); // Scale from 1.0 to 0.7
      ...
      // Add opacity fade effect to the entire section
      const opacity = 1 - (easeOutCubic * 0.4); // Fade from 1.0 to 0.6
  ...
  // Initialize the carousel shrink effect
  window.addEventListener('load', initHeroCarouselShrink);
